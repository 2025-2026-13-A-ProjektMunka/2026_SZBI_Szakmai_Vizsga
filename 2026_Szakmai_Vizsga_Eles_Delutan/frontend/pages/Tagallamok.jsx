    ...
    useEffect(() => {
        const letolt = async () => {
            const response = await fetch(
                'http://localhost:3800/api/frontend/tagallamok',
            );

            const adat = await response.json();

            if (response.ok) {
                setTagorszagok(adat.tagallamok);
            } else {
                ...
                {tagorszagok.map((elem) => (
                    ...
                    >
                        <div className="szoveg-kontener">
                            ...
                            <p>Haderő nagysága: {elem.haderoNagysaga} fő</p>
                        </div>
